...
class Wheelleg : public LibXR::Application {
 public:
  ...
  Wheelleg(LibXR::HardwareContainer& hw, LibXR::ApplicationManager& app,
    ...
    this->hip_motor_.at(3) = new DMMotor(hw, app, hip_rightback_param);
    ...
    event_handler_.Register(
        static_cast<uint32_t>(WheellegEvent::SET_MODE_RELAX), event_callback);
    event_handler_.Register(
        static_cast<uint32_t>(WheellegEvent::SET_MODE_STAND), event_callback);
    event_handler_.Register(
        static_cast<uint32_t>(WheellegEvent::SET_MODE_ROTOR), event_callback);
    event_handler_.Register(
        static_cast<uint32_t>(WheellegEvent::SET_MODE_RESET), event_callback);
  ...
  void UpdateFeedback() {
    ...
    bool left_online = this->wheel_motor_[0]->Update() == LibXR::ErrorCode::OK;
    bool right_online = this->wheel_motor_[1]->Update() == LibXR::ErrorCode::OK;

    bool wheel_online_now = left_online and right_online;
    if (!wheel_online_now) {
      ...
      body_argum_.C620_ready_flag_ =
          (now_ - wheel_online_start_time_ >= 2500000ULL);
    }

    /* 虚拟腿角度计算 */
    /* 左腿前关节角度 */
    leg_argum_[0].phi4_ = RangeAnglePI(hip_motor_[0]->GetFeedback().abs_angle -
                                       param_.mech_zero[0]);
    leg_argum_[0].omega4_ = hip_motor_[0]->GetFeedback().omega;

    /* 左腿后关节角度 */
    leg_argum_[0].phi1_ = RangeAnglePI(hip_motor_[1]->GetFeedback().abs_angle -
                                       param_.mech_zero[1]);
    ...
    /* 右腿前关节角度 */
    leg_argum_[1].phi4_ = RangeAnglePI(
        -(hip_motor_[2]->GetFeedback().abs_angle - param_.mech_zero[2]));
    leg_argum_[1].omega4_ = -hip_motor_[2]->GetFeedback().omega;

    /* 右腿后关节角度 */
    leg_argum_[1].phi1_ =
        RangeAnglePI(-(hip_motor_[3]->GetFeedback().abs_angle + 2.19448972f));
    ...
    auto result0 = vmc_left_->VMCsolve(
        -leg_argum_[0].phi1_, -leg_argum_[0].phi4_, -body_argum_.pit_,
        body_argum_.gyro_, -leg_argum_[0].omega1_, -leg_argum_[0].omega4_, dt_);
    ...
    auto result1 = vmc_right_->VMCsolve(
        -leg_argum_[1].phi1_, -leg_argum_[1].phi4_, -body_argum_.pit_,
        body_argum_.gyro_, -leg_argum_[1].omega1_, -leg_argum_[1].omega4_, dt_);
    ...
    leg_argum_[0].single_x_dot =
        -wheel_motor_[0]->GetFeedback().omega / 15.765f * param_.wheel_radius;
    /*右侧轮速*/
    leg_argum_[1].single_x_dot =
        wheel_motor_[1]->GetFeedback().omega / 15.765f * param_.wheel_radius;
    /* 平均速度估计 */
    body_argum_.x_dot =
        (leg_argum_[0].single_x_dot + leg_argum_[1].single_x_dot) / 2.0f -
        3.14e-07f;

    /* 机体角速度 俯视逆时针旋转为正 */
    body_argum_.wz = (-(leg_argum_[0].single_x_dot + leg_xdot_0) +
                      (leg_argum_[1].single_x_dot + leg_xdot_1)) /
                     0.46f;

    /*避免陀螺仪上电尖峰加速度*/
    if (now_ > 1000000 and (current_mode_ == STAND or current_mode_ == JUMP)) {
      /*调用自适应滤波*/
      body_argum_.x_dot_hat =
          AdaptFilter(
              body_argum_.wz, gyro_.z,
              (body_argum_.x_dot + (leg_xdot_0 + leg_xdot_1) / 2.0f),
              accl_.y * GRAVITY + fabsf(gyro_.z) * fabsf(gyro_.z) * 0.25f,
              dt_) -
    ...
    if (body_argum_.limit_power == 0 or body_argum_.limit_power >= 500) {
      body_argum_.limit_power = 50;
    }
    ...
    body_argum_.expspeed =
        std::clamp(0.5f / (leg_argum_[0].L0 + leg_argum_[1].L0 + 0.2f) + 1.45f,
                   0.5f, param_.max_speed);
    ...
    if (body_argum_.cap_energy <= 0.3f) {
      ...
      float speed_limit =
          min_speed + (max_speed_limit - min_speed) * energy_ratio;
      body_argum_.expspeed = std::min(body_argum_.expspeed, speed_limit);
    }

    if (chassis_cmd_.self_define == CMD::ChasStat::BOOST and
        body_argum_.cap_energy > 0.4f) {
  ...
  void Calculate() {
    ...
    switch (current_mode_) {
      ...
      case JUMP: {
        ...
        body_argum_.target_yaw_dot =
            RampTowards(body_argum_.target_yaw_dot, 0.0f, 0.05f);
        if (fabsf(chassis_cmd_.x) > 0.05f) {
          body_argum_.target_yaw =
              atan2f(chassis_cmd_.y, chassis_cmd_.x) - 1.571f;
        ...
        body_argum_.target_dot_x =
            RampTowards(body_argum_.target_dot_x,
                        (fabsf(chassis_cmd_.x) / 2.5f + chassis_cmd_.y) *
                            cosf(body_argum_.target_yaw + body_argum_.yaw_) *
                            body_argum_.expspeed,
                        0.005f);
        body_argum_.target_dot_x =
            std::clamp(body_argum_.target_dot_x,
      ...
      case ROTOR:
        // TODO:旋转时直线速度规划
        body_argum_.target_x = 0.0f;
        if (fabsf(chassis_cmd_.y) > 0.05f or fabsf(chassis_cmd_.x) > 0.05f) {
          body_argum_.xhat = 0.0f;
        }
        body_argum_.target_dot_x = sinf(body_argum_.yaw_) * chassis_cmd_.y *
                                       body_argum_.expspeed / 2.5f -
                                   cosf(body_argum_.yaw_) * chassis_cmd_.x *
                                       body_argum_.expspeed / 2.5f;
        body_argum_.target_dot_x =
            std::clamp(body_argum_.target_dot_x, -body_argum_.expspeed / 2.5f,
                       body_argum_.expspeed / 2.5f);
        body_argum_.target_yaw_dot = RampTowards(
            body_argum_.target_yaw_dot,
            body_argum_.expspeed / (0.23f * 1.0f) -
                fabsf(std::clamp(
                    fabsf(chassis_cmd_.y * body_argum_.expspeed / 2.5f) +
                        fabsf(chassis_cmd_.x * body_argum_.expspeed / 2.5f),
                    0.0f, body_argum_.expspeed / 2.5f)) /
                    (1.2f * 0.23f),
            0.045f);
        ...
        if (chassis_cmd_.self_define == CMD::ChasStat::BOOST) {
          ...
          body_argum_.target_yaw_dot = RampTowards(
              body_argum_.target_yaw_dot, body_argum_.expspeed / 0.23f, 0.035f);
        }
        body_argum_.target_yaw = 0.0f;
        break;
      case RESET:
    ...
    leg_argum_[0].Fn = vmc_left_->GndDetector(
        hip_motor_[0]->GetFeedback().torque,
        hip_motor_[1]->GetFeedback().torque, accl_.z * GRAVITY,
        leg_argum_[0].theta, leg_argum_[0].d_theta, this->dt_);

    leg_argum_[1].Fn = vmc_right_->GndDetector(
        hip_motor_[3]->GetFeedback().torque,
        hip_motor_[2]->GetFeedback().torque, accl_.z * GRAVITY,
        leg_argum_[1].theta, leg_argum_[1].d_theta, this->dt_);
    ...
    if (current_mode_ == RELAX || current_mode_ == ROTOR ||
        current_mode_ == STAND || current_mode_ == JUMP) {
      /* 处理k矩阵 */
      for (int i = 0; i < 40; i++) {
        body_argum_.LQR_K[i] =
            vmc_left_->Lqr2KCalc(&this->param_.K_Poly_Coefficient[i][0],
                                 leg_argum_[0].L0, leg_argum_[1].L0);
      }
      float s_limit_k = 1.0f;
      float yaw_limit_k =
          3.14f * 1.1f / ((leg_argum_[0].L0 + leg_argum_[1].L0) / 0.3f);
      ...
      if (fabsf(body_argum_.x_dot_hat * gyro_.z) > 6.0f) {
        for (int i = 0; i < 4; i++) {
          body_argum_.LQR_K[0 + i * 10] =
              6.0f / fabsf(body_argum_.x_dot_hat * gyro_.z) *
              body_argum_.LQR_K[0 + i * 10];
      ...
      body_argum_.thetaK_offset_ =
          -0.31476241584544784 * leg_x + 0.21558012163788323;
      ...
      if (leg_argum_[0].onground_flag_) {
        ...
        leg_argum_[0].Tw_s_lim = std::clamp(
            body_argum_.LQR_K[0] * (-body_argum_.xhat + body_argum_.target_x +
        ...
        leg_argum_[0].Tw_yaw_lim = std::clamp(
            (body_argum_.LQR_K[2] *
                 (body_argum_.yaw_ + body_argum_.target_yaw) +
        ...
        leg_argum_[0].Tw =
            (leg_argum_[0].Tw_s_lim + leg_argum_[0].Tw_yaw_lim +
             ...
             body_argum_.LQR_K[8] *
                 (-body_argum_.pit_ + body_argum_.pitK_offset_) +
             body_argum_.LQR_K[9] * (-body_argum_.gyro_ + 0.0f));
        /*防打滑力矩*/
        leg_argum_[0].Tw_adapt =
            0.2f * (leg_argum_[0].x_dot_pred - leg_argum_[0].single_x_dot) /
            param_.wheel_radius;

        /*------------------------- 左腿 -----------------------*/
        /*位移输出*/
        leg_argum_[0].Tp_s_lim = std::clamp(
            body_argum_.LQR_K[20] * (-body_argum_.xhat + body_argum_.target_x +
                                     body_argum_.sK_offset) +
      ...
      if (leg_argum_[1].onground_flag_) {
        ...
        leg_argum_[1].Tw_s_lim = std::clamp(
            body_argum_.LQR_K[10] * (-body_argum_.xhat + body_argum_.target_x +
                                     body_argum_.sK_offset) +
        ...
        leg_argum_[1].Tw =
            (leg_argum_[1].Tw_s_lim + leg_argum_[1].Tw_yaw_lim +
             ...
             body_argum_.LQR_K[18] *
                 (-body_argum_.pit_ + body_argum_.pitK_offset_) +
             body_argum_.LQR_K[19] * (-body_argum_.gyro_ + 0.0f));
        /*防打滑力矩*/
        leg_argum_[1].Tw_adapt =
            0.2f * (leg_argum_[1].x_dot_pred - leg_argum_[1].single_x_dot) /
            param_.wheel_radius;

        /*------------------------- 右腿 -----------------------*/
        /*位移输出*/
        leg_argum_[1].Tp_s_lim = std::clamp(
            body_argum_.LQR_K[30] * (-body_argum_.xhat + body_argum_.target_x +
                                     body_argum_.sK_offset) +
      ...
      if (chassis_cmd_.self_define == CMD::ChasStat::STRETCH) {
        ...
        /*用于飞坡 下台阶 走坡面*/
        if (body_argum_.leg_change_time > 1) {
          leg_argum_[0].target_delta_l0 = 0.08f;
          leg_argum_[1].target_delta_l0 = 0.08f;
          body_argum_.stair_ready_flag = false;
        }
        /*用于上台阶*/
        if (body_argum_.leg_change_time > 200) {
          leg_argum_[0].target_delta_l0 = 0.21f;
          leg_argum_[1].target_delta_l0 = 0.21f;
          body_argum_.leg_change_time = 1001;
      ...
      if (body_argum_.stair_ready_flag and leg_argum_[0].theta > 0.5f and
          leg_argum_[1].theta > 0.5f and current_mode_ == STAND) {
        /* 上台阶处理 */
        body_argum_.leg_change_time = 0;
        SetMode(STAIR);
      }

      leg_argum_[0].Delta_L0 = RampTowards(
          leg_argum_[0].Delta_L0, leg_argum_[0].target_delta_l0, 0.01f);
      leg_argum_[1].Delta_L0 = RampTowards(
          leg_argum_[1].Delta_L0, leg_argum_[1].target_delta_l0, 0.01f);
      ...
      // }
      leg_argum_[0].Delta_L0 = std::clamp(leg_argum_[0].Delta_L0, -0.06f,
                                          0.4f - param_.static_L0[0]);
      leg_argum_[1].Delta_L0 = std::clamp(leg_argum_[1].Delta_L0, -0.06f,
                                          0.4f - param_.static_L0[1]);
      ...
      leg_argum_[0].F0 =
          leg_argum_[0].Delta_F +
          leglength_pid_left_.Calculate(
              leg_argum_[0].Delta_L0 + param_.static_L0[0], leg_argum_[0].L0,
      ...
      leg_argum_[1].F0 =
          leg_argum_[1].Delta_F +
          leglength_pid_right_.Calculate(
              leg_argum_[1].Delta_L0 + param_.static_L0[1], leg_argum_[1].L0,
      ...
      if (current_mode_ == JUMP) {
        ...
        if (body_argum_.jump_time < 150000)
        /*跳跃预备 收腿*/
        {
          leg_argum_[0].F0 =
              leg_argum_[0].Delta_F +
              leglength_pid_left_.Calculate(0.12f, leg_argum_[0].L0,
                                            leg_argum_[0].d_L0, dt_) -
              gyro_.z * body_argum_.x_dot_hat * (leg_argum_[0].L0) / 0.23f *
                  (8.0f + 0.5f * 0.7f * leg_argum_[0].L0) +
              body_argum_.roll_out;
          ...
          leg_argum_[1].F0 =
              ...
              leglength_pid_right_.Calculate(0.12f, leg_argum_[1].L0,
                                             leg_argum_[1].d_L0, dt_) +
              gyro_.z * body_argum_.x_dot_hat * (leg_argum_[1].L0) / 0.23f *
                  (8.0f + 0.5f * 0.7f * leg_argum_[1].L0) -
              body_argum_.roll_out;
        } else if (body_argum_.jump_time < 370000)
        /*起跳*/
        {
          leg_argum_[0].F0 =
              vmc_left_->MaxFnSolve(26) + leg_argum_[0].spring_force;
          leg_argum_[1].F0 =
              vmc_right_->MaxFnSolve(26) + leg_argum_[1].spring_force;
        } else if (body_argum_.jump_time < 520000)
        /*起跳收腿 增加离地高度*/
        {
          if (leg_argum_[0].L0 < 0.14) {
            leg_argum_[0].F0 =
                vmc_left_->MaxFnSolve(-7) + leg_argum_[0].spring_force;
          } else {
            leg_argum_[0].F0 =
                vmc_left_->MaxFnSolve(-24) + leg_argum_[0].spring_force;
          }

          if (leg_argum_[1].L0 < 0.14) {
            leg_argum_[1].F0 =
                vmc_right_->MaxFnSolve(-7) + leg_argum_[1].spring_force;
          } else {
            leg_argum_[1].F0 =
                vmc_right_->MaxFnSolve(-24) + leg_argum_[1].spring_force;
          }
          ...
          leg_argum_[0].F0 =
              std::clamp(12.0f / (leg_argum_[0].L0 - 0.135f) + 20, 0.0f,
                         param_.static_F0[0] * 2.0f);
          leg_argum_[1].F0 =
              std::clamp(12.0f / (leg_argum_[1].L0 - 0.135f) + 20, 0.0f,
                         param_.static_F0[1] * 2.0f);

          for (int i = 0; i < 4; i++) {
            dmmotor_cmd_[i].kd = 0.8f;
          }
      ...
      auto result3 =
          vmc_left_->VMCinserve(-leg_argum_[0].phi1_, -leg_argum_[0].phi4_,
      ...
      if (body_argum_.pit_ > 0.87f) {
        ...
        if (eulr_.rol > 0.1f) {
          ...
          if (fabsf(leg_argum_[0].target_theta + 1.57f / 4) < 0.05f) {
            leg_argum_[1].target_theta =
                RampForaward(leg_argum_[1].target_theta, -1.57f / 4, 0.002f);
            leg_argum_[1].target_theta =
                RangeAnglePI(leg_argum_[1].target_theta);
          } else {
            leg_argum_[0].target_theta =
                RampForaward(leg_argum_[0].target_theta, -1.57f / 4, 0.002f);
            leg_argum_[0].target_theta =
                RangeAnglePI(leg_argum_[0].target_theta);
          }

          /*两条腿一起发力*/

        } else if (eulr_.rol < -0.1f) {
          if (fabsf(leg_argum_[1].target_theta + 1.57f / 4) < 0.05f) {
            leg_argum_[0].target_theta =
                RampForaward(leg_argum_[0].target_theta, -1.57f / 4, 0.002f);
            leg_argum_[0].target_theta =
                RangeAnglePI(leg_argum_[0].target_theta);
          ...
          leg_argum_[1].target_theta =
              RampForaward(leg_argum_[1].target_theta, 0.0f, 0.002f);
          leg_argum_[0].target_theta =
              RampForaward(leg_argum_[0].target_theta, 0.0f, 0.002f);
        ...
        if (eulr_.rol > 0.1f) {
          ...
          if (fabsf(leg_argum_[0].target_theta - 1.57f / 4) < 0.05f) {
            leg_argum_[1].target_theta =
                RampBack(leg_argum_[1].target_theta, 1.57f / 4, 0.002f);
            leg_argum_[1].target_theta =
                RangeAnglePI(leg_argum_[1].target_theta);
          } else {
            leg_argum_[0].target_theta =
                RampBack(leg_argum_[0].target_theta, 1.57f / 4, 0.002f);
            leg_argum_[0].target_theta =
                RangeAnglePI(leg_argum_[0].target_theta);
          }
          ...
          leg_argum_[1].target_theta =
              RampBack(leg_argum_[1].target_theta, 0.0f, 0.002f);
          leg_argum_[0].target_theta =
              RampBack(leg_argum_[0].target_theta, 0.0f, 0.002f);
          leg_argum_[1].target_theta = RangeAnglePI(leg_argum_[1].target_theta);
          leg_argum_[0].target_theta = RangeAnglePI(leg_argum_[0].target_theta);
        }
        ...
        if (leg_argum_[0].theta < -1.57 / 2 or leg_argum_[0].theta > 1.57) {
          leg_argum_[0].target_theta =
              RampBack(leg_argum_[0].target_theta, 0.0f, 0.005f);
          leg_argum_[0].target_theta = RangeAnglePI(leg_argum_[0].target_theta);
          ...
          leg_argum_[0].Delta_L0 =
              std::clamp(leg_argum_[0].Delta_L0, -0.03f, 0.34f);
          leg_argum_[0].F0 =
              -20.0f + leglength_pid_left_.Calculate(
                           leg_argum_[0].Delta_L0, this->leg_argum_[0].L0,
                           this->leg_argum_[0].d_L0, this->dt_);
        }
        ...
        if (leg_argum_[1].theta < -1.57 / 2 or leg_argum_[1].theta > 1.57) {
          leg_argum_[1].target_theta =
              RampBack(leg_argum_[1].target_theta, 0.0f, 0.005f);
          leg_argum_[1].target_theta = RangeAnglePI(leg_argum_[1].target_theta);
          ...
          leg_argum_[1].Delta_L0 =
              std::clamp(leg_argum_[1].Delta_L0, -0.03f, 0.34f);
          leg_argum_[1].F0 =
              -20.0f + leglength_pid_right_.Calculate(
                           leg_argum_[1].Delta_L0, this->leg_argum_[1].L0,
                           this->leg_argum_[1].d_L0, this->dt_);
        }
      }
      auto result3 = vmc_left_->VMCinserve(
          -leg_argum_[0].phi1_, -leg_argum_[0].phi4_, -leg_argum_[0].Tp,
          leg_argum_[0].F0 - leg_argum_[0].spring_force);
      leg_argum_[0].T1 = std::get<0>(result3);
      leg_argum_[0].T2 = std::get<1>(result3);
      ...
      switch (body_argum_.stair_phase) {
        ...
        case 1:
          leg_argum_[0].Delta_L0 =
              RampTowards(leg_argum_[0].Delta_L0, 0.12f, 0.002f);
          leg_argum_[1].Delta_L0 =
              RampTowards(leg_argum_[1].Delta_L0, 0.12f, 0.002f);
          if (leg_argum_[0].Delta_L0 < 0.121f and
              leg_argum_[1].Delta_L0 < 0.121f) {
            body_argum_.stair_phase = 2;
          }
          break;
        case 2:
          leg_argum_[0].target_theta =
              RampTowards(leg_argum_[0].target_theta, 0.0f, 0.01f);
          leg_argum_[1].target_theta =
              RampTowards(leg_argum_[1].target_theta, 0.0f, 0.01f);
          if (fabsf(leg_argum_[0].target_theta) < 0.015f and
              fabsf(leg_argum_[1].target_theta) < 0.015f) {
            SetMode(RESET);
          }
          break;
        default:
      ...
      leg_argum_[0].Tp =
          15 * theta_pid_left_.Calculate(leg_argum_[0].target_theta,
                                         leg_argum_[0].theta, dt_);
      leg_argum_[1].Tp =
          15 * theta_pid_right_.Calculate(leg_argum_[1].target_theta,
                                          leg_argum_[1].theta, dt_);
      leg_argum_[0].F0 = 1.5f * leglength_pid_left_.Calculate(
                                    leg_argum_[0].Delta_L0, leg_argum_[0].L0,
                                    leg_argum_[0].d_L0, dt_);
      leg_argum_[1].F0 = 1.5f * leglength_pid_right_.Calculate(
                                    leg_argum_[1].Delta_L0, leg_argum_[1].L0,
                                    leg_argum_[1].d_L0, dt_);
      ...
      case ROTOR:
        ...
        if (!body_argum_.C620_ready_flag_ or
            (this->hip_motor_[0]->GetFeedback().state != 1 or
             ...
             this->hip_motor_[3]->GetFeedback().state != 1) or
            (fabsf(leg_argum_[0].theta) > 1.57f / 1.5f or
             fabsf(leg_argum_[1].theta) > 1.57f / 1.5f) or
            fabsf(body_argum_.pit_) > 1.0f) {
          this->wheel_motor_[0]->Relax();
          this->wheel_motor_[1]->Relax();
          this->hip_motor_[0]->Disable();
          this->hip_motor_[1]->Disable();
      ...
      case RESET:
        ...
        if (body_argum_.C620_ready_flag_) {
          ...
          if (fabsf(leg_argum_[0].theta) < 0.15f and
              fabsf(leg_argum_[1].theta) < 0.15f and
              leg_argum_[0].L0 < 0.17f and leg_argum_[1].L0 < 0.17f and
              body_argum_.pit_ < 1.0f and
              (this->hip_motor_[0]->GetFeedback().state == 1 and
               ...
               this->hip_motor_[3]->GetFeedback().state == 1)) {
            SetMode(STAND);
          }
        }
  ...
  void InitUi() {
    ...
    switch (ui_step_) {
      ...
      case 1: {
        uint16_t dot_x =
            static_cast<uint16_t>(1920 / 2 + 260 * sinf(body_argum_.yaw_));
        uint16_t dot_y =
            static_cast<uint16_t>(1080 / 2 + 260 * cosf(body_argum_.yaw_));
        Referee::UIFigure fig{};
        referee_->FillCircle(fig, "WY", ADD_OP, UI_LAYER_CHASSIS,
                             Referee::UIColor::UI_COLOR_CYAN, 7, dot_x, dot_y,
                             20);
      ...
      case 2: {
        ...
        uint16_t pit_cx = static_cast<uint16_t>(
            1690 - PIT_LEN * sinf(-body_argum_.pit_ - 1.57f));
        uint16_t pit_cy = static_cast<uint16_t>(
            480 + PIT_LEN * cosf(-body_argum_.pit_ - 1.57f));
        uint16_t pit_x2 = static_cast<uint16_t>(
            1690 + PIT_LEN * sinf(-body_argum_.pit_ - 1.57f));
        uint16_t pit_y2 = static_cast<uint16_t>(
            480 - PIT_LEN * cosf(-body_argum_.pit_ - 1.57f));

        Referee::UIFigure pit_fig{};
        referee_->FillLine(pit_fig, "WP", ADD_OP, UI_LAYER_CHASSIS,
                           Referee::UIColor::UI_COLOR_CYAN, 2, pit_cx, pit_cy,
                           pit_x2, pit_y2);
      ...
      case 3: {
        ...
        uint16_t leg_y2 =
            static_cast<uint16_t>(leg_y1 - leg_px * cosf(leg_argum_[0].theta));

        Referee::UIFigure leg_fig{};
        referee_->FillLine(leg_fig, "WL", ADD_OP, UI_LAYER_CHASSIS,
                           Referee::UIColor::UI_COLOR_ORANGE, 3, leg_x1, leg_y1,
                           leg_x2, leg_y2);
      ...
      case 4: {
        ...
        uint16_t leg_y4 =
            static_cast<uint16_t>(leg_y3 - leg_px1 * cosf(leg_argum_[1].theta));

        Referee::UIFigure leg_fig{};
        referee_->FillLine(leg_fig, "WR", ADD_OP, UI_LAYER_CHASSIS,
                           Referee::UIColor::UI_COLOR_GREEN, 3, leg_x3, leg_y3,
                           leg_x4, leg_y4);
        referee_->SendUIFigure(id, client, leg_fig);
      ...
      case 5: {
        if (ADD_OP == Referee::UIFigureOp::UI_OP_ADD) {
          ...
          referee_->FillLine(line_fig, "WSL", ADD_OP, UI_LAYER_CHASSIS,
                             Referee::UIColor::UI_COLOR_CYAN, 1, 0, 0, 560,
                             500);
      ...
      case 6: {
        if (ADD_OP == Referee::UIFigureOp::UI_OP_ADD) {
          ...
          referee_->FillLine(line_fig, "WSR", ADD_OP, UI_LAYER_CHASSIS,
                             Referee::UIColor::UI_COLOR_CYAN, 1, 1920, 0, 1360,
                             500);
          referee_->SendUIFigure(id, client, line_fig);
        }
        break;
      ...
      case 7: {
        ...
        Referee::UIColor cap_state = Referee::UIColor::UI_COLOR_CYAN;
        if (body_argum_.cap_energy < 0.6f) {
          cap_state = Referee::UIColor::UI_COLOR_YELLOW;
        }
        if (body_argum_.cap_energy < 0.3f) {
        ...
        referee_->FillLine(
            spfig, "SP", ADD_OP, UI_LAYER_CHASSIS, cap_state, 20, 400, 200,
            static_cast<uint16_t>(400 + 1120 * body_argum_.cap_energy), 200);
  ...
  void EventHandler(uint32_t event_id) {
    switch (static_cast<WheellegEvent>(event_id)) {
      case WheellegEvent::SET_MODE_RELAX:
        SetMode(Mode::RELAX);
        break;
      case WheellegEvent::SET_MODE_STAND:
        SetMode(Mode::STAND);
        break;
      case WheellegEvent::SET_MODE_ROTOR:
        SetMode(Mode::ROTOR);
        break;
      case WheellegEvent::SET_MODE_RESET:
        SetMode(Mode::RESET);
        break;
      case WheellegEvent::SET_MODE_JUMP:
        SetMode(Mode::JUMP);
        break;
      default:
        break;
  ...
  void SetMode(Mode mode) {
    ...
    if (mode == STAND or mode == ROTOR) {
      this->event_handler_.Active(Wheellegmode::NOW_MODE_MOVE);
    }
    if (mode == RESET) {
      this->event_handler_.Active(Wheellegmode::NOW_MODE_RESET);
    }
  ...
  float AdaptFilter(float wz, float gyro_z, float speed, float accl,
    ...
    adaptfilter_argum_.k1 =
        0.00182238811503557f *
        powf(M_E, 8.78806145711193f * adaptfilter_argum_.error / 3.0f);
    adaptfilter_argum_.k1 = std::clamp(adaptfilter_argum_.k1, 0.0f, 1.0f);
    adaptfilter_argum_.xhat =
        speed + adaptfilter_argum_.k1 * (adaptfilter_argum_.xhatminus - speed);
    return adaptfilter_argum_.xhat;
  }

  float RangeAnglePI(float angle) {
    while (angle > static_cast<float>(LibXR::PI)) {
      angle -= static_cast<float>(LibXR::TWO_PI);
    }
    while (angle < -static_cast<float>(LibXR::PI)) {
      angle += static_cast<float>(LibXR::TWO_PI);
    }
 ...
 private:
  ...
  struct {
    ...
    uint8_t yaw_switch_time = 0;
    bool yaw_switch_flag = false;
    /*离地相关*/
