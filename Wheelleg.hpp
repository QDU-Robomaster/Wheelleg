...
void UpdateFeedback() {
    ...
    /* 消除腿摆动导致位移的变化 */
    float leg_x_0 = leg_argu_[0].L0 * leg_argu_[0].d_theta * cosf(leg_argu_[0].theta) + leg_argu_[0].d_L0 * sinf(leg_argu_[0].theta);
    float leg_x_1 = leg_argu_[1].L0 * leg_argu_[1].d_theta * cosf(leg_argu_[1].theta) + leg_argu_[1].d_L0 * sinf(leg_argu_[1].theta);
    /*注意减速比 两者均向前+*/
    /*左侧轮速*/
    ...
    move_argu_.x_dot = (leg_argu_[0].single_x_dot + leg_argu_[1].single_x_dot) / 2.0f;
    move_argu_.x_dot_2 = (move_argu_.x_dot - move_argu_.last_x_dot) / dt_;
    /* 机体角速度 俯视逆时针旋转为正 */
    move_argu_.wz = (-(leg_argu_[0].single_x_dot + leg_x_0) + (leg_argu_[1].single_x_dot + leg_x_1)) / 0.46f;

    /*避免陀螺仪上电尖峰加速度*/
    if (now_>1000000) {
    /*调用自适应滤波*/
        move_argu_.x_dot_hat = AdaptFilter(
        move_argu_.wz, gyro_.z, move_argu_.x_dot, accl_.y * 9.8f, dt_);
...
void Calculate() {
  ...
  switch (current_mode_) {
   ...
   /* 命令应该控加速度而不是速度? */
   /* 平移速度计算 */
   /* 0.002为最大加速度 即333hz*0.002 梯度式递增减 */

   if (fabs(move_argu_.target_dot_x - chassis_cmd_.y * cosf(yaw_) * param_.max_speed) > 0.006) {
    if (move_argu_.target_dot_x > chassis_cmd_.y * cosf(yaw_) * param_.max_speed) {
          move_argu_.target_dot_x -= 0.005;}
    if (move_argu_.target_dot_x < chassis_cmd_.y * cosf(yaw_) * param_.max_speed) {
          move_argu_.target_dot_x += 0.005;}
   } else {
    ...
    move_argu_.target_yaw_dot = yaw_dot_;
    /*双零点*/
   ...
   case ROTOR:
    ...
    move_argu_.target_yaw += 0.007f;
    move_argu_.target_yaw_dot = 0.0007*333.0f;
    break;
   case RESET:
     move_argu_.target_dot_x = 0.0f;
     move_argu_.target_x = 0.0f;
  ...
  if (current_mode_ == RELAX || current_mode_ == ROTOR || current_mode_ == STAND) {
     ...
     /* K增益计算 */
  if (onground0_flag_) {
    leg_argu_[0].Tw = (move_argu_.LQR_K[0] * (-move_argu_.xhat + move_argu_.target_x + 1.21f) +
      move_argu_.LQR_K[1] * (-move_argu_.x_dot_hat + move_argu_.target_dot_x) +
      move_argu_.LQR_K[2] * (yaw_ + 0.0f) +
    ...
    leg_argu_[0].Tp = (move_argu_.LQR_K[20] * (-move_argu_.xhat + move_argu_.target_x + 1.21f) +
      ...
      move_argu_.LQR_K[28] * (-pit_ + 0.05F) +
     ...
     leg_argu_[0].Tp = (move_argu_.LQR_K[24] * (-leg_argu_[0].theta + 0.0f) +
       ...
       move_argu_.LQR_K[27] * (-leg_argu_[1].d_theta + 0.0f)); }
  if (onground1_flag_) {
     leg_argu_[1].Tw = (move_argu_.LQR_K[10] * (-move_argu_.xhat + move_argu_.target_x + 1.21f) +
      move_argu_.LQR_K[11] * (-move_argu_.x_dot_hat + move_argu_.target_dot_x) +
      move_argu_.LQR_K[12] * (yaw_ + 0.0f) +
     ...
     leg_argu_[1].Tp = (move_argu_.LQR_K[30] * (-move_argu_.xhat + move_argu_.target_x + 1.21f) +
       ...
       move_argu_.LQR_K[38] * (-pit_ + 0.05f) +
 ...
 leg_argu_[1].Delta_L0 = param_.static_L0[1] - roll_pid_.Calculate(0.0f, eulr_.rol, dt_);
  ...
  wheel_motor_out_[1] = (leg_argu_[1].Tw + leg_argu_[1].Tw_adapt);
   /*偏执腿力矩补偿*/
  this->leg_argu_[0].Delta_Tp = 1.7f;
  this->leg_argu_[1].Delta_Tp = 1.7f;
